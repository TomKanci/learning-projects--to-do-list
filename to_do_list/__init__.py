...
def get_tasks(task_type, checked=False):
    ...
    if task_type == "id" and not checked:
        ...
        return (Task.query
                    .filter_by(task_type=task_type, checked=checked)
...
def index():
    ...
    return render_template('index.html', index_page=index_page, tasks_id=tasks_id, 
...
def add_task():
    """
    Route for adding a new task. If the form is valid on submission, a new task is created and added to the database.
...
def save_order():
    """
    Route for saving the order of tasks. The order and group of tasks are received as JSON in the request.
    The order and group of each task are updated in the database.

    Returns:
    str, int: An empty response with a 204 status code, indicating that the request has succeeded but there's no additional content to send in the response body.
    """
    group_to_task_type = {
        'box-today': 'Today',
        'box-week': 'This week',
...
def update_task(task_id):
    """
    Route for updating the checked status of a task. The task to be updated is identified by the task_id in the URL.
